    ...
    <>
      <Box flexDirection="column" rowGap={1}>
        {recentEntries.length === 0 ?
          ...
          :
          <Box><Text>Recent 7 days</Text></Box>
        }
        {recentEntries.map((entry, index) =>
          <Box flexBasis="row" columnGap={4} key={index}>
            <Box><Text>{entry.date === today ? 'Today' : entry.date}</Text></Box>
            <Box><Text>{formatNumberToLiter(getTotalVolume(entry.store.volumes))}</Text></Box>
          </Box>
        )}
      </Box>
    </>
